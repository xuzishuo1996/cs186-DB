...
//            rids.remove(removePos);
//        }
        if (keys.contains(key)) {
            rids.remove(getKey(key).get());
            keys.remove(key);   // no duplicates
        }
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        // (proj2): implemented and passed public test - TestLeafNode::testToAndFromBytes
        ...
        // verify that this is a LeafNode
        byte nodeType = buf.get();
        assert (nodeType == (byte) 1);

        // deserialize rightSibling
        long rightSiblingId = buf.getLong();
        Optional<Long> rightSibling = (rightSiblingId == -1L) ? Optional.empty() : Optional.of(rightSiblingId);
        ...
        // use the constructor that reuses an existing page instead of fetching a brand new one
